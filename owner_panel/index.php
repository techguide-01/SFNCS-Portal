...
<body>
  ...
  <li class="nav-item">
    ...
    <div class="card3">
        <div class="card" style="width: 18rem; border: 2px solid #ffc107;">
             <div class="card-body text-center">
               <h3 class="mb-3 text-warning"><i class="fa-solid fa-file-circle-check"></i></h3>
               <h5 class="card-title">Grade Approval</h5>
               <?php 
                 $count_q = mysqli_query($conn, "SELECT id FROM published_grades WHERE status = 'PENDING'");
                 $pending_count = mysqli_num_rows($count_q);
               ?>
               <p class="card-text">Mayroong <b><?php echo $pending_count; ?></b> student grades na naghihintay ng approval.</p>
               <a href="approve_grades.php" class="btn btn-warning fw-bold">
                  Review Grades 
                  <?php if($pending_count > 0) echo "<span class='badge bg-danger'>$pending_count</span>"; ?>
               </a>
             </div>
        </div>
    </div>
